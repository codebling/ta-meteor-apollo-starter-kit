...
async function renderAsync() {
  ...
  const WrapWithRoutingSwitch = ({ component }) => (
    <Switch>
  ...
  const ClientApp = () => (
    <Router history={history}>
      <ApolloProvider client={client}>
        <header className="fixed top-0 z1">
          <div className="flex justify-between items-center">
            <div className="flex justify-center items-center header__burger header__burger--show">
              <svg className="menu__icon no--select" width="24px" height="24px" viewBox="0 0 48 48" fill="#fff">
                <path d="M6 36h36v-4H6v4zm0-10h36v-4H6v4zm0-14v4h36v-4H6z" />
              </svg>
            </div>
            <div id="header-title" className="center header__title no--select">
              <WrapWithRoutingSwitch component={HeaderTitle} />
            </div>
            <div id="header-lang-picker" className="no--select">
              <WrapWithRoutingSwitch component={LanguagePicker} />
            </div>
          </div>
        </header>

        <div id="burger-btn-controller" />

        <div className="fixed top-0 bottom-0 z2 menu">
          <div className="menu__header" />
          <ul id="menu" className="menu__list">
            <WrapWithRoutingSwitch component={Menu} />
          </ul>
        </div>

        <div className="fixed top-0 bottom-0 left-0 right-0 z1 menu__overlay" />

        <div className="flex justify-around items-center app__outer">
          <div className="app__container pt4">
            <div className="flex flex-column justify-between">
              <WrapWithRoutingSwitch component={Routes} />
            </div>
          </div>
        </div>
      </ApolloProvider>
